...
## Codebase Intelligence for ProgressBarsOpen (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-15 (commit 4df770c). Confidence: 70%.
...
### Architecture
**Files:** 56 | **Lines:** 10992
ProgressBarsOpen is a json codebase of 56 files. Execution starts at server.py. Start here when reading the codebase. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on.

### Key modules
- `root` — Repository root
**Language:** python | **Files:** 2 | **Public symbols:** 12 / 34
...
- `road-progress.html` — 5 commits/90d
- `server.py` — 3 commits/90d
- `road-progress-data.json` — 4 commits/90d

### Code health
Three co-equal signals: defect risk 9.22/10 avg, hotspot health 6.27/10 (stable), worst `server.py` at 3.88/10 · maintainability 9.88/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `road-progress-data.json` — change entropy — impact −3.0
- `road-progress.html` — change entropy — impact −3.0
- `server.py` — change entropy — impact −3.0
