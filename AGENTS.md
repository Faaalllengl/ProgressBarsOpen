...
## Codebase Intelligence for ProgressBar's (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-13 (commit 226c51a). Confidence: 65%.
...
### Architecture
**Files:** 12 | **Lines:** 3030
ProgressBar's is a json codebase of 12 files. Execution starts at server.py. Start here when reading the codebase. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on.

### Key modules
- `root` — Repository root
**Language:** python | **Files:** 2 | **Public symbols:** 15 / 37
...
- `road-progress.html` — 6 commits/90d

### Code health
Three co-equal signals: defect risk 9.68/10 avg, hotspot health 10.0/10 (stable), worst `server.py` at 6.01/10 · maintainability 9.69/10 · performance risk 0 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
