...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-25 (commit e0f69fb). Confidence: 72%.
...
### Architecture
**Files:** 8 | **Lines:** 4508
ProgressBarsOpen is a python codebase of 8 files. Execution starts at server.py. Start here when reading the codebase. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on.
...
- `road-progress.html` — 12 commits/90d
- `road-progress-data.json` — 6 commits/90d
- `.repowise/knowledge-graph.json` — 3 commits/90d
- `server.py` — 8 commits/90d

### Code health
Three co-equal signals: defect risk 6.58/10 avg, hotspot health 6.28/10 (stable), worst `server.py` at 5.26/10 · maintainability 9.12/10 · performance risk 2 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
