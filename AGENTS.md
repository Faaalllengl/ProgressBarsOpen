...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-25 (commit e870e5c). Confidence: 72%.
...
### Architecture
**Files:** 10 | **Lines:** 4526
ProgressBarsOpen is a json codebase of 10 files. Execution starts at server.py. Start here when reading the codebase. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on.

### Key modules
- `root` — scripts
**Language:** python | **Files:** 3 | **Public symbols:** 19 / 53
Covers the 3 source files in 2 directories under root

### Entry points
- `server.py`

### Files that need care (bug-fix history first, then churn — check `get_risk` before editing)
- `road-progress.html` — 11 commits/90d
- `road-progress-data.json` — 6 commits/90d
- `.repowise/knowledge-graph.json` — 3 commits/90d
- `server.py` — 8 commits/90d

### Code health
Three co-equal signals: defect risk 6.12/10 avg, hotspot health 5.74/10 (stable), worst `server.py` at 2.46/10 · maintainability 9.09/10 · performance risk 2 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `road-progress.html` — change entropy — impact −2.6
- `server.py` — change entropy — impact −2.6
